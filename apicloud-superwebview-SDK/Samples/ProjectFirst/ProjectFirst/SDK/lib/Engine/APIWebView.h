...
/*!
 ...
 @param callback 回调方法
 @param ret 表示成功的参数
 @param delete 是否执行回调后在js端删除该function，若只需要回调一次则传YES，便于内存回收
 @param err 表示错误的参数
 */
- (void)sendResultWithCallback:(NSInteger)callback ret:(NSDictionary *)ret err:(NSDictionary *)err delete:(BOOL)delete;
...
- (void)webView:(APIWebView *)webView didFailLoadWithError:(nullable NSError *)error;
